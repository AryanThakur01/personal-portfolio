      ...
      <div className="max-w-[1240px] mx-auto px-[22px] sm:px-8">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-8 mb-10">
          ...
          <p className="max-w-[420px] text-text-2 text-[14px] m-0">
            Each card is a real service running somewhere. Live indicator = service is responding to /health right now.
          </p>
        </div>
          ...
          {LAB.map((s, i) => (
            ...
            >
              ...
              <div className="flex justify-between items-center gap-3">
                ...
                <span className={`font-mono text-[10px] tracking-[0.14em] uppercase inline-flex items-center gap-[6px] shrink-0 ${s.live ? "text-green" : "text-text-3"}`}>
                  <span
                    className={`w-[7px] h-[7px] rounded-full shrink-0 ${s.live ? "bg-green" : "bg-text-4"}`}
                    style={s.live ? { boxShadow: "0 0 0 3px rgba(34,197,94,0.15)", animation: "pulse 2.2s infinite" } : undefined}
                  />
              ...
              >
                <b className="text-text-2 font-medium">{s.pattern.b}</b>{s.pattern.rest}
              </div>
              ...
              <div className="flex gap-[10px] mt-1 pt-3 border-t border-dashed border-border-strong">
                ...
                <a href="#" className="no-underline font-mono text-[11px] text-text-2 inline-flex items-center gap-[6px] hover:text-accent transition-colors duration-150 ml-auto">
                  github ↗
                </a>
              </div>
            </div>
